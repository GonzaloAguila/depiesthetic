...
const HomeLayout = () => {
  ...
  useEffect(() => {
    ...
    selectedZones.forEach((zone) => {
      zones.forEach((zoneGroup) => {
        const selectedZone = zoneGroup.items.find(
  ...
  const priceRow = (label: string, price: number) => <div></div>;
        ...
        <div className={`${styles.contentContainer} ml-2 mr-2`}>
              ...
              <div className="flex w-full  mt-2 justify-center   md:flex-row">
                <div className={`flex justify-center items-center mr-2`}>
                  <RadioButton
                    ...
                    className={`${styles.radiobutton} ${
                      "F" === selectedGender ? styles.selectedRadiobutton : ""
                    }`}
                ...
                <div className={`flex items-center mr-2`}>
                  <RadioButton
                    ...
                    value={"M"}
                    className={`${styles.radiobutton} ${
                      "M" === selectedGender ? styles.selectedRadiobutton : ""
                    }`}
                    onChange={(e) => setSelectedGender(e.value)}
          ...
          <div className={styles.totalPriceContainer}>
            ...
            {selectedZones.map((zone, index) => (
              <div key={index} className={styles.zoneContainer}>
                <p>{bodyDictionary[zone]}</p>
                <p>
                  $
                  {
                    zones
                      .find((z) => z.items.find((item) => item.value === zone))
                      ?.items.find((item) => item.value === zone)?.price
                  }
                  ,00
                </p>
              </div>
